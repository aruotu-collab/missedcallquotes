...
export function SettingsForm({ business }: { business: Business }) {
  ...
  return (
    <form onSubmit={onSubmit} className="mt-8 grid gap-4 text-sm">
      ...
      <p className="rounded-xl border border-dash-line bg-white p-4 text-dash-muted">
      ...
      <button className="rounded-full bg-dash-accent py-3 font-medium text-white">Save setup</button>
      {saved ? <p className="text-dash-accent">Saved.</p> : null}
    </form>
  );
}
...
function Field(props: React.InputHTMLAttributes<HTMLInputElement> & { label: string; name: string }) {
  ...
  return (
    <label className="grid gap-1">
      ...
      <input {...rest} className="rounded-xl border border-dash-line bg-white px-3 py-2.5 text-dash-ink" />
    </label>
  );
}
